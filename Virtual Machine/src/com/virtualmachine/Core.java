	...
	public class InstructionRunner implements Runnable {
		...
		public void run() {
			...
			synchronized (core) {
				...
				parentWindow.cod_maq.setRowSelectionInterval(programCounter,
						programCounter);
				parentWindow.cod_maq.scrollRectToVisible(new Rectangle(
						parentWindow.cod_maq.getCellRect(programCounter, 0, true)));
				...
				if (commands.get(stepNumber).equals("LDC")) {
					...
					parentWindow.saida.append(memory[stackTop] + "\n");
					stackTop -= 1;
